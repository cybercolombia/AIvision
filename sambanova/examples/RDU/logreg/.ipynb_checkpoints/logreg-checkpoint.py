...
class LogReg(nn.Module):
    ...
    def online_inference(self, args: argparse.Namespace, mock=False):
        ...
        while queue:
            ...
            output = self.lin_layer(input_tensor)
            for i in range(self.num_linear - 1):
                output = self.lin_layer(output)

            output = samba.to_torch(output.unsqueeze(0))
...
def add_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument('--enable-runtime-profiler', action='store_true', help='Enable Samba Runtime Profiler')
...
def test(args: argparse.Namespace, model: nn.Module, inputs: Tuple[samba.SambaTensor],
    ...
    if not args.inference:
        ...
        for name, param in model.named_parameters():
            ...
            utils.assert_close(golden_gradient,
                               samba_gradient,
                               f'Gradient for {name}',
                               threshold=3e-3,
...
def main(argv):
    ...
    if args.command == "compile":
        ...
        if args.enable_runtime_profiler:
